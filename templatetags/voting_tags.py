...
class VoteForm(template.Node):
    ...
    def render(self, context):
        ...
        vote = Vote.objects.get_score(object)
        
        vote = math.ceil(vote['score'])
        ...
        context.push()
        formstr = render_to_string(template_search_list, c, context)
        context.pop()
                
        return formstr
        
def my_render_comment_form(parser, token):
    return MyRenderCommentFormNode.handle_token(parser, token)
...
def render_vote_form(parser, token):
...
register.tag('render_vote_form', render_vote_form)
